...
public class MidpointDisplacement
{
    ...
    public void Generate(Random random)
    {
        ...
        while (_passes.TryPop(out var nextPass))
        {
            DoPass(nextPass.X, nextPass.Y, nextPass.Size, random);
        }
    }
    ...
    private void DoPass(int x, int y, int size, Random random)
    {
        ...
        SetHeight(center, (_heightmap[topMidpoint] + _heightmap[bottomMidpoint] + _heightmap[leftMidpoint] + _heightmap[rightMidpoint]) * 0.25f + RandomJitter(random));
    ...
    private float RandomHeight(Random random) => random.NextSingle() * Amplitude;
    private float RandomJitter(Random random) => random.NextSingle() * JitterAmplitude - JitterAmplitude * 0.5f;
    ...
    private int MidpointIndex(int a, int b) => a + (b - a) / 2;
}